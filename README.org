...
The package is useful when writing the first draft of something, not so much in the editing and rewriting phases where, alas, hard work seldom is reflected in the word count.
...
For that reason it's a good idea to put a =nowc= or =noexport= tag on any heading not containing whatever you consider productive writing, such as research and todo lists – and the heading containing the table itself, so the tracking of word counts won't influence the actual word count.
...
If an entry from the current day already exist, =org-tt-write= will update this entry instead of creating a new one. 
In that way, =org-tracktable.el= aims to keep only one entry per day. For testing purposes, you can manually change the time-stamp for the last entry to get familiar with the behavior.
...
- The percentage of your daily writing goal achieved so far. The daily writing goal is set to 300 by default. It can be altered by changing the variable =org-tt-daily-goal=. Set to 0 to disable 'org-tt-status' from displaying daily goal.
